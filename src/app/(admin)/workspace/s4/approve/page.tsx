...
function HitlBadge({ hitlStatus }: { hitlStatus: string | null }) {
  ...
  return (
    ...
    >
      ...
      Gate S4 — {hitlStatus.charAt(0).toUpperCase() + hitlStatus.slice(1)}
    </span>
  );
}
...
function S4ApproveContent() {
  ...
  const loadRun = useCallback(
    async (id?: string) => {
      ...
      setRun(null);
      try {
        const res = await fetch(`${API_BASE}/v1/acp/gate/s4/run/${rid}`, {
          headers: authHeaders(),
        });
        if (res.status === 401) {
          setToken(null);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="bg-white rounded-lg shadow p-8 w-full max-w-sm">
          ...
          <h1 className="text-lg font-semibold mb-4">S4 Blog Drafts Approval</h1>
          {loginError && (
  ...
  async function handleApprove() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API_BASE}/v1/acp/gate/s4/approve`, {
        method: "POST",
        headers: authHeaders(),
      ...
      setApproveNotes("");
      setToast("S4 gate approved. Audit log written.");
  ...
  async function handleReject() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API_BASE}/v1/acp/gate/s4/reject`, {
        method: "POST",
        headers: authHeaders(),
      ...
      setRejectNotes("");
      setRejectError("");
      setToast("S4 gate rejected. Notes recorded in audit log.");
      await loadRun(run.run_id);
    } catch (e: unknown) {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="flex items-start justify-between mb-6 gap-4">
        <div>
          <h1 className="text-xl font-semibold">S4 Blog Drafts Approval</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Review blog draft output and approve or reject
          </p>
        </div>
      ...
      {showApproveModal && (
        <div className="fixed inset-0 z-40 bg-black/40 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <h2 className="text-base font-semibold mb-2">Approve S4 Gate</h2>
            <p className="text-sm text-gray-600 mb-1">
              Confirm approval of blog draft output for this run.
            </p>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 z-40 bg-black/40 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <h2 className="text-base font-semibold mb-1">Reject S4 Gate</h2>
...
export default function S4ApprovePage() {
  return (
